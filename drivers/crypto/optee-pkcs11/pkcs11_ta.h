...
#define PKCS11_CK_UNAVAILABLE_INFORMATION   0xFFFFFFFFU
#define PKCS11_UNDEFINED_ID		    0xFFFFFFFFU
...
struct optee_pkcs11_slot_info {
	...
	u32 flags;
	u8 hardware_version[PKCS11_SLOT_VERSION_SIZE];
	u8 firmware_version[PKCS11_SLOT_VERSION_SIZE];
};
...
struct pkcs11_token_info {
	u8 label[PKCS11_TOKEN_LABEL_SIZE];
	u8 manufacturer_id[PKCS11_TOKEN_MANUFACTURER_SIZE];
	u8 model[PKCS11_TOKEN_MODEL_SIZE];
	u8 serial_number[PKCS11_TOKEN_SERIALNUM_SIZE];
	u32 flags;
	u32 max_session_count;
	u32 session_count;
	u32 max_rw_session_count;
	u32 rw_session_count;
	u32 max_pin_len;
	u32 min_pin_len;
	u32 total_public_memory;
	u32 free_public_memory;
	u32 total_private_memory;
	u32 free_private_memory;
