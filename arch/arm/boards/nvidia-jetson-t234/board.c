...
static int nvidia_jetson_t23x_fs_init(void)
{
	struct i2c_client client;
	u8 data;

	if (!of_machine_is_compatible("nvidia,jetson-t23x"))
		return 0;
...
fs_initcall(nvidia_jetson_t23x_fs_init);

static int nvidia_jetson_t23x_device_init(void)
{
	if (!of_machine_is_compatible("nvidia,jetson-t23x"))
		return 0;
...
device_initcall(nvidia_jetson_t23x_device_init);
