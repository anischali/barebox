 ...
 */
#define pr_fmt(fmt)     "optee-tk: " fmt
#include <linux/err.h>
...
int trusted_tee_get_random(unsigned char *key, size_t key_len)
{
	...
	ret = tee_client_invoke_func(pvt_data.ctx, &inv_arg, param);
	if ((ret < 0) || (inv_arg.ret != 0)) {
		pr_err("TA_CMD_GET_RANDOM invoke err: %x\n",
			inv_arg.ret);
		ret = -EFAULT;
	} else {
...
static void trusted_key_remove(struct device *dev)
{
...
static int __init optee_tk_mod_init(void)
{
	return driver_register(&trusted_key_driver.driver);
}
device_initcall(optee_tk_mod_init);

MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("Sumit Garg <sumit.garg@linaro.org>");
MODULE_AUTHOR("Anis Chali <chalianis1@gmail.com>");
MODULE_DESCRIPTION("OP-TEE trusted keys");
